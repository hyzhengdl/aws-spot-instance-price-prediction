...
'''
predict a long time later
'''

offset = 12 # 12*5=60, a hour later
total = 30000
...
def get_batch(batch_size):
    ...
    for i in range(batch_size):
        s = random.randint(0,total-n_steps*n_input-offset-1)
        for j in range(n_steps*n_input):
            batch_x[i,j] = train_data[s+j]
        # print s,n_steps*n_input
        batch_y[i,0]=train_data[s+n_steps*n_input+offset-1]
    return batch_x, batch_y


def test_batch():
    size = len(test_data) - n_input*n_steps - offset
    ...
    for i in range(size):
        ...
        batch_y[i,0] = test_data[i+n_input*n_steps+offset-1]
    return batch_x,batch_y
...
with tf.Session() as sess:
    ...
    for step in range(steps+1):
        ...
        if step % 1200 == 0 and step>0:
            ...
            dir = os.path.join('model1', sys.argv[1], sys.argv[2])
    ...
    print 'finished!!!'
    with open(os.path.join(sys.argv[1], '1-'+sys.argv[2]+'-pred.csv'), 'w') as f:
        f.writelines('real,prediction\n')
        for i in range(len(test_data)-n_steps*n_input-offset):
